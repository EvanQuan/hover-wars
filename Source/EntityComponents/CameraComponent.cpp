...
CameraComponent::CameraComponent(int iEntityID, int iComponentID)
	: EntityComponent( iEntityID, iComponentID )
{
	m_vPos = DEFAULT_POS;
	m_vWorldLookAt = vec3(0.f, 5.f, 0.f);		// (X, Y, Z)
	updateHxW(START_HEIGHT, START_WIDTH);
	m_m4Frame = mat4(1.0);
...
mat4 CameraComponent::getToCameraMat()
{
...
// Generates toCamera Matrix and updates Uniform in ShaderManager.
mat4 CameraComponent::getPerspectiveMat()
{
	//vec3 vCamCPos = getCartesianPos();
...
void CameraComponent::orbit(vec2 pDelta)
{
	if (!m_bSteady)
	{
		THETA += pDelta.x;
...
void CameraComponent::zoom(float fDelta)
{
