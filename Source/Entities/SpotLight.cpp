...
void SpotLight::initialize(float fPhi, float fSoftPhi, bool bStatic, const vec3* vColor, const vec3* vDirection, const string& sMeshLocation, const ObjectInfo* pObjectProperties, float m_fMeshScale)
{
    ...
    // Load Mesh
    if ("" == sMeshLocation)
        m_pMesh = MESH_MANAGER->generateSphereMesh(bStatic, 0.25f, sMaterial, pBoundingBox, m_vPosition);
    else
        m_pMesh = MESH_MANAGER->loadMeshFromFile(sMeshLocation, sMaterial, pBoundingBox, m_fMeshScale, m_vPosition, bStatic);
